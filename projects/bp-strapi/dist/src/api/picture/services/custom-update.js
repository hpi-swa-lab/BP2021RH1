...
const updatePreviousOrMergeWithExistingDescription = async (pictureQuery, currentPictureId, descriptionQuery, previousDescriptionId, newDescriptionData) => {
    ...
    if (!newDescriptionAlreadyInDb) {
        ...
        if (newDescriptionAlreadyInDb.id === previousDescriptionId)
            return;
        ...
        strapi.log.debug(`The merging process will also effect the following pictures: [${newPicturesForTheExistingDescription.map((o) => o.id)}]`);
...
const deletePreviousTagIfNeeded = async (pictureQuery, tagQuery, tagKeyInPictureRelation, previousTagId, tagHasVerifiedHandling = true) => {
    ...
    if (picturesWithPreviousTag.length >= 2)
        return;
...
const processUpdatesForDescriptions = async (pictureQuery, currentPictureId, data) => {
    ...
    if (!data[DESCRIPTIONS_KEY])
        return;
    const previousDescriptions = (_b = (_a = (await pictureQuery.findOne({
        where: {
            id: currentPictureId,
        },
        populate: {
    ...
    }))) === null || _a === void 0 ? void 0 : _a.descriptions) !== null && _b !== void 0 ? _b : [];
    ...
    for (const description of data[DESCRIPTIONS_KEY]) {
        ...
        if (description.updatePrevious) {
            newDescriptionId = await updatePreviousOrMergeWithExistingDescription(pictureQuery, currentPictureId, tagQuery, description.id, newDescriptionData);
        }
        else {
        ...
        if (Number(newDescriptionId) !== Number(description.id)) {
            await deletePreviousTagIfNeeded(pictureQuery, tagQuery, DESCRIPTIONS_KEY, description.id, false);
        }
    }
    ...
    for (const unusedId of unused) {
        await deletePreviousTagIfNeeded(pictureQuery, tagQuery, DESCRIPTIONS_KEY, unusedId, false);
    }
...
const processUpdatesForTimeRangeTag = async (pictureQuery, data) => {
    ...
    data[withVerifiedPrefix(TIME_RANGE_TAG_KEY)] = timeRangeTag.verified
        ? newTimeRangeTagId
        : null;
    strapi.log.debug(`New ${timeRangeTag.verified ? "verified " : ""}${TIME_RANGE_TAG_KEY}: ${newTimeRangeTagId}`);
    ...
    if (Number(timeRangeTag.id) !== Number(newTimeRangeTagId)) {
        await deletePreviousTagIfNeeded(pictureQuery, tagQuery, TIME_RANGE_TAG_KEY, timeRangeTag.id, true);
    }
};
/**
 ...
 */
const processSimpleTagRelationUpdates = async (tagKeyInPictureRelation, data) => {
    // Check whether we actually need to update stuff for that tag type.
    if (!data[tagKeyInPictureRelation])
        return;
    const newTags = [];
...
const processUpdatesForKeywordTags = async (data) => processSimpleTagRelationUpdates(KEYWORD_TAGS_KEY, data);
const processUpdatesForLocationTags = async (data) => processSimpleTagRelationUpdates(LOCATION_TAGS_KEY, data);
const processUpdatesForPersonTags = async (data) => processSimpleTagRelationUpdates(PERSON_TAGS_KEY, data);
...
const insertCrossProductIgnoreDuplicates = async (knexEngine, linksTable, leftIdKey, rightIdKey, leftIds, rightIds) => {
    ...
    await knexEngine(linksTable)
        .insert(knexEngine.select(leftIdKey, rightIdKey)
...
const bulkEditTimeRangeTag = async (knexEngine, pictureQuery, pictureIds, data) => {
    ...
    for (const [linksTable, dataKey] of [
        ...
        if (newTimeRangeTagId !== null) {
            await knexEngine(linksTable)
                .insert(knexEngine.select("picture_id", knexEngine.raw("?", [newTimeRangeTagId]))
...
const bulkEditArchiveTag = async (knexEngine, pictureIds, data) => {
    ...
    if (newArchiveId !== null) {
        await knexEngine(linksTable)
            .insert(knexEngine.select("picture_id", knexEngine.raw("?", [newArchiveId]))
            .from(knexIdArray(knexEngine, pictureIds, "a(picture_id)")));
    }
    return true;
};
const bulkEditDescriptions = async (knexEngine, pictureIds, data) => {
    ...
    for (const added of diff.added) {
        ...
        const newId = await findExistingOrCreateNewTag(tagQuery, tagService, DESCRIPTIONS_KEY, newData);
        addedIds.push(newId);
    }
